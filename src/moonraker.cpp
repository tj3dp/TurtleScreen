...
void MOONRAKER::get_T3_status(void) {
    String T3_status = send_request("GET", "/printer/objects/query?gcode_macro%20_T3_STATUS");
    if (!T3_status.isEmpty()) {
        DynamicJsonDocument json_parse(T3_status.length() * 2);
        deserializeJson(json_parse, T3_status);
        data.homing = json_parse["result"]["status"]["gcode_macro _T3_STATUS"]["homing"].as<bool>();
        data.probing = json_parse["result"]["status"]["gcode_macro _T3_STATUS"]["probing"].as<bool>();
        data.qgling = json_parse["result"]["status"]["gcode_macro _T3_STATUS"]["qgling"].as<bool>();
        data.heating_nozzle = json_parse["result"]["status"]["gcode_macro _T3_STATUS"]["heating_nozzle"].as<bool>();
        data.heating_bed = json_parse["result"]["status"]["gcode_macro _T3_STATUS"]["heating_bed"].as<bool>();
#ifdef MOONRAKER_DEBUG
        Serial.print("homing: ");
        ...
        Serial.println("Empty: moonraker: get_T3_status");
    }
}
...
void moonraker_task(void * parameter) {
    ...
    if(WiFi.isConnected())Serial.println("Wifi is on trying to kick off moonraker loop");
    for(;;) {
        if (WiFi.isConnected()) {
